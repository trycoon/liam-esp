 ...
 * @param <bool> inSeries if true the delay time for a scheduled function will be releative the previous scheduled
 * function. If false the delay will be relative the current time when the function was scheduled.
 */
Scheduler::Scheduler(bool inSeries)
    : in_series(inSeries)
{
}
 ...
 * @param <bool> repeat when the delay has been reached and the function has been executed, then reschedule the function
 * for another delay milliseconds.
 */
...
void Scheduler::process()
{
    ...
    {
        // Check if delay has expired. Handles timer overflow. millis() on ESP8266 has a roll over of 72 minutes. (Based
        // on microsecond tick.)

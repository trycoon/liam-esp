...
Cutter::Cutter(IO_Analog& io_analog)
    : cutter_id(3)
    , io_analog(io_analog)
{
...
void Cutter::start()
{
    if (cutterSpeed == 0)
    {
        cutterLoadReadingTicker.attach_ms<Cutter*>(100, [](Cutter* instance) { instance->senseLoad(); }, this);
...
void Cutter::process()
{
    // slowly ramp cutter speed up to reach target ("cutterSpeed"), this is to save fuses and electronics from current
    // surges.

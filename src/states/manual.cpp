...
Manual::Manual(Definitions::MOWER_STATES myState, StateController& stateController, Resources& resources)
    : AbstractState(myState, stateController, resources)
{
}
...
void Manual::process()
{
    // if we drive manually and get disconnected from mower or mower loose WiFi connectivity, then stop mower from
    // driving further.
    ...
    // if we have parked in dockingstation manually and mower detects it's docked, then enter docked-state after a short
    // timeout.

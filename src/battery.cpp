...
Battery::Battery(IO_Analog& io_analog, TwoWire& w)
    : io_analog(io_analog)
    , wire(w)
{
}
...
void Battery::updateChargeCurrent()
{
    ...
    // we can get some missreadings (1475.10) from time to time, so we record samples to an array an take the median
    // value to filter out all noice.

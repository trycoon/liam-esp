...
bool Dockingstation::transmit(uint8_t* buffer, size_t length)
{
    ...
    {
        Log.trace(F("Ongoing LoRa transmission detected, waiting a moment and trying "
                    "again." CR));
        delay(random(20, 25));
    ...
    if (state != RADIOLIB_ERR_NONE)
    {
        Log.warning(F("LoRa unknown transmission fault: %d. Ignoring transmission." CR), state);
        return false;
    }

...
/**
 * Class for interfacing with a ultrasound sonar sensor, used for detecting obstacles around the mower before we bump
 * into them. Compatible sensors are HC-SR04. SRF06 is NOT supported since it's too slow.
 */
Sonar::Sonar()
{
...
void Sonar::process()
{
    ...
    {
        // if we have not received an response within 30ms since ping started then either our startsignal was not
        // detected by the sonar-sensor or no obstacle was detected within 4,5 meters range.
        pingInProgress = false;
    }
    ...
    if (!pingInProgress)
    {
        ...
        //  Log.notice("t1: %d, t2: %d, d: %d %d %d %d %d" CR, test1, test2, sonar.sampleDistances[0],
        //  sonar.sampleDistances[1], sonar.sampleDistances[2], sonar.sampleDistances[3], sonar.sampleDistances[4]);

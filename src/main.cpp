...
// Give us an warning if main loop is delayed more than this value. This could
// be an indication of long hidden "delay" calls in our code.
...
void check_SPI() {
  Log.notice(F("SPI pins, MOSI: %d, MISO: %d, SCK: %d, SS: %d." CR), MOSI, MISO,
             SCK, SS);
}
...
void setup() {
  ...
  Log.notice(F(CR "=== %s v%s ===\nbuild time: %s %s\nCPU: %dx%d MHz\nFlash: "
                  "%d KiB\nChip revision: %d\n=======================" CR CR),
             Definitions::APP_NAME, Definitions::APP_VERSION, __DATE__,
             __TIME__, chip_info.cores, ESP.getCpuFreqMHz(),
             ESP.getFlashChipSize() / 1024, chip_info.revision);
  ...
  // initialize state controller, assume we are DOCKED unless there is a saved
  // state.
  if (rtc_get_reset_reason(0) == SW_CPU_RESET && lastState.length() > 0) {
    Log.notice(F("Returning to last state \"%s\" after software crash!" CR),
               lastState.c_str());
...
void loop() {
  ...
  if (Configuration::config.setupDone) {
    ...
    if (io_accelerometer.isFlipped() &&
        stateController.getStateInstance()->getState() !=
            Definitions::MOWER_STATES::FLIPPED) {
  ...
  if (loopDelay > LOOP_DELAY_WARNING &&
      (currentTime - loopDelayWarningTime) > LOOP_DELAY_WARNING_COOLDOWN) {
    loopDelayWarningTime = currentTime;

    Log.warning(F("Main loop running slow due to long delay(%l us)! Make sure "
                  "thread is not blocked by delays()." CR),
                (uint32_t)loopDelay);
  }
